...
private:
	...
	Enemy* find_target_enemy()
	{
		...
		{
		case Archer:
			view_range = instance->archer_template.range[instance->level_archer];
			break;
		case Axeman:
			view_range = instance->axeman_template.range[instance->level_axeman];
			break;
		case Gunner:
			view_range = instance->gunner_template.range[instance->level_gunner];
			break;
		}
